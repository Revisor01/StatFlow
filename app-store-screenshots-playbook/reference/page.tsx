...
type Copy = { de: { headline: string; subline: string }; en: { headline: string; subline: string } };
...
const LAYOUT_ROTATION: Layout[] = [
  ...
  "headline-top", // 07 Mehrere Roboter
  "mock-left",    // 08 Consumables
...
function sourcePath(id: string, locale: "de" | "en"): string {
  if (id === "01-live-map") return `/screenshots/${locale}/01-live-map.png`;
  const num = id.slice(0, 2);
  return `/screenshots/${locale}/${num}-placeholder.png`;
}
...
function DeviceMockup({
  ...
  // The Kit's mockup.png has a transparent screen area.
  // Place the source screenshot behind it with the same aspect ratio.
  // Source 1179x2556 (6.1"), mockup 1022x2082. The screen area of the
  // mockup is roughly ~92% width and ~95% height, centered.
  ...
  return (
    ...
    >
      {/* Screenshot behind mockup frame */}
      <div
        style={{
          position: "absolute",
          left: screenInsetX,
    ...
    >
      ...
      {slide.layout === "mock-podest" && (
        <>
          ...
          >
            <DeviceMockup screenshot={slide.screenshot} scaleOverride={1.05} />
          </div>
          {/* Subline bottom */}
          <div
            style={{
      ...
      {slide.layout === "mock-edge" && (
        <>
          ...
          >
            <DeviceMockup screenshot={slide.screenshot} scaleOverride={1.35} />
          </div>
          {/* Headline top */}
          <div
            style={{
